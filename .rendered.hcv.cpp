...
//#define pymat(in, out) //	pyptr(in,uint8_t);cv::Mat out; //	switch (PASTE(in,_).shape[2]) //	{ case 1: out = cv::Mat(PASTE(in,_).shape[0], PASTE(in,_).shape[1], CV_8UC1, PASTE(in,__));break; //	case 3: 	out = cv::Mat(PASTE(in,_).shape[0], PASTE(in,_).shape[1], CV_8UC3, PASTE(in,__));break;}
